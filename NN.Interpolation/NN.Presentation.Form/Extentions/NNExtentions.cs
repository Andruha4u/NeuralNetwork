...
namespace NN.Presentation.Form.Extentions
{
    ...
    {
        public static void ToNN(this TreeView t, ApproximationNeuralNetwork nn)
        {
            t.Nodes.Clear();
        ...
        public static void ToNN(this PictureBox p, ApproximationNeuralNetwork nn, int X, int Y)
        {
            int neuronWidth = 30;
            ...
            {
                BaseLayer<double, double> layer = nn.Layers[l];

                int x = X - (neuronDistance * (layer.Neurons.Count / 2));

                for (int n = 0; n < layer.Neurons.Count; n++)
                {
                    BaseNeuron<double> neuron = layer.Neurons[n];
